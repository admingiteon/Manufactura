...
view: alm_lp_pt_almacenamiento {
  derived_table: {
    sql:
      ...
      select centro,fecha,nombre,capacidad_total_ubicacion valor,'CAPACIDAD DE ALMACENAJE' concepto,1 idconcepto from psa-psa-cadena-qa.modelo_de_calculo.LP_PT_Almacenamiento
      union all
      select centro,fecha,nombre,ocupacion_inicial valor,'OCUPACION INICIAL' concepto,2 idconcepto from psa-psa-cadena-qa.modelo_de_calculo.LP_PT_Almacenamiento
       union all
      select centro,fecha,nombre,0 valor,'ENTRADAS' concepto,3 idconcepto from psa-psa-cadena-qa.modelo_de_calculo.LP_PT_Almacenamiento
             union all
      select centro,fecha,nombre,0 valor,'SALIDAS' concepto,4 idconcepto from psa-psa-cadena-qa.modelo_de_calculo.LP_PT_Almacenamiento
      union all
      ...
      union all
      select centro,fecha,nombre,case when  ocupacion_final > capacidad_total_ubicacion then capacidad_total_ubicacion /NULLIF(ocupacion_final,0) else 0 end valor,'FALTANTE DE CAPACIDAD DE ALMACENAJE (%)' concepto,8 idconcepto from psa-psa-cadena-qa.modelo_de_calculo.LP_PT_Almacenamiento
       union all
